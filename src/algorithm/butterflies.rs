...
impl<T: FftNum> Butterfly5<T> {
    ...
    unsafe fn perform_fft_contiguous(&self, mut buffer: impl LoadStore<T>) {
        unsafe {
            ...
            let b14re_a =
                buffer.load(0).re + self.twiddle1.re * x14p.re + self.twiddle2.re * x23p.re;
            let b14re_b = self.twiddle1.im * x14n.im + self.twiddle2.im * x23n.im;
            let b23re_a =
                buffer.load(0).re + self.twiddle2.re * x14p.re + self.twiddle1.re * x23p.re;
            let b23re_b = self.twiddle2.im * x14n.im + -self.twiddle1.im * x23n.im;

            let b14im_a =
                buffer.load(0).im + self.twiddle1.re * x14p.im + self.twiddle2.re * x23p.im;
            let b14im_b = self.twiddle1.im * x14n.re + self.twiddle2.im * x23n.re;
            let b23im_a =
                buffer.load(0).im + self.twiddle2.re * x14p.im + self.twiddle1.re * x23p.im;
...
impl<T: FftNum> Butterfly7<T> {
    ...
    unsafe fn perform_fft_contiguous(&self, mut buffer: impl LoadStore<T>) {
        unsafe {
            ...
            let x16re_b = self.twiddle1.im * x16n.im
                + self.twiddle2.im * x25n.im
                + self.twiddle3.im * x34n.im;
            let x25re_a = buffer.load(0).re
                + self.twiddle1.re * x34p.re
                + self.twiddle2.re * x16p.re
                + self.twiddle3.re * x25p.re;
            let x25re_b = -self.twiddle1.im * x34n.im + self.twiddle2.im * x16n.im
                - self.twiddle3.im * x25n.im;
            let x34re_a = buffer.load(0).re
                + self.twiddle1.re * x25p.re
                + self.twiddle2.re * x34p.re
                + self.twiddle3.re * x16p.re;
            let x34re_b = -self.twiddle1.im * x25n.im
                + self.twiddle2.im * x34n.im
                + self.twiddle3.im * x16n.im;
            let x16im_a = buffer.load(0).im
                + self.twiddle1.re * x16p.im
                + self.twiddle2.re * x25p.im
                + self.twiddle3.re * x34p.im;
            let x16im_b = self.twiddle1.im * x16n.re
                + self.twiddle2.im * x25n.re
                + self.twiddle3.im * x34n.re;
            let x25im_a = buffer.load(0).im
                + self.twiddle1.re * x34p.im
                + self.twiddle2.re * x16p.im
                + self.twiddle3.re * x25p.im;
            let x25im_b = -self.twiddle1.im * x34n.re + self.twiddle2.im * x16n.re
                - self.twiddle3.im * x25n.re;
            let x34im_a = buffer.load(0).im
                + self.twiddle1.re * x25p.im
                + self.twiddle2.re * x34p.im
                + self.twiddle3.re * x16p.im;
            let x34im_b = self.twiddle1.im * x25n.re
                - self.twiddle2.im * x34n.re
                - self.twiddle3.im * x16n.re;
...
impl<T: FftNum> Butterfly8<T> {
    ...
    unsafe fn perform_fft_contiguous(&self, mut buffer: impl LoadStore<T>) {
        unsafe {
            ...
            scratch1[1] =
                (twiddles::rotate_90(scratch1[1], self.direction) + scratch1[1]) * self.root2;
            scratch1[2] = twiddles::rotate_90(scratch1[2], self.direction);
            scratch1[3] =
                (twiddles::rotate_90(scratch1[3], self.direction) - scratch1[3]) * self.root2;
...
impl<T: FftNum> Butterfly9<T> {
    ...
    unsafe fn perform_fft_contiguous(&self, mut buffer: impl LoadStore<T>) {
        unsafe {
            ...
            // step 5: row FFTs
            self.butterfly3.perform_fft_strided(
                &mut scratch0[0],
                &mut scratch1[0],
                &mut scratch2[0],
            );
...
impl<T: FftNum> Butterfly24<T> {
    ...
    unsafe fn perform_fft_contiguous(&self, mut buffer: impl LoadStore<T>) {
        unsafe {
            ...
            scratch3[5] = (twiddles::rotate_90(scratch3[5], self.fft_direction()) + scratch3[5])
                * -self.root2;
