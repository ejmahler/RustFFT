...
fn cooley_tukey(signal: &[Complex<f32>],
    ...
    match factors {
        [] => {
            for i in range_step(0, signal.len(), signal_stride) {
                unsafe { *scratch.get_unchecked_mut(i) = *signal.get_unchecked(i) };
            }
        },
        [n1, other_factors..] => {
            ...
            for i in range(0, n1)
            {
                cooley_tukey(signal.slice_from(i), signal_stride * n1,
                             scratch.slice_from_mut(i), scratch_stride * n1,
                             spectrum.slice_from_mut(i), spectrum_stride * n1,
...
fn butterfly(input: &[Complex<f32>],
    ...
    for (i, in_row) in input.chunks(num_cols * input_stride).enumerate() {
        let out_col = output.slice_from_mut(i);
        ...
        for (j, spec_bin_idx) in range_step(0, out_col.len(), out_col_stride).enumerate() {
            ...
            *spec_bin = Zero::zero();
            for (k, sig_bin_idx) in range_step(0, in_row.len(), input_stride).enumerate() {
                let sig_bin = unsafe { input.get_unchecked(sig_bin_idx) };
