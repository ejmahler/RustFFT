...
use algorithm::{FFTAlgorithm, CooleyTukey, Radix4, RadersAlgorithm, GoodThomasAlgorithm, NoopAlgorithm};
use math_utils;
	...
	where T: Signed + FromPrimitive + Copy + Debug + 'static
{
	if len < 2 {
    	...
    	//good-thomas does have some overhead, so we only want to do this if it's a non trivial size
    	if smallest_factor == 2 && smallest_count > 4  {
    		...
    		let p2_fft = Box::new(Radix4::new(1 << smallest_count, inverse)) as Box<FFTAlgorithm<T>>;
    		let other_fft = plan_fft_with_factors(len >> smallest_count, &factors[1..], inverse);

    		Box::new(GoodThomasAlgorithm::new(1 << smallest_count, p2_fft, len >> smallest_count, other_fft)) as Box<FFTAlgorithm<T>>
    	} else {
...
fn plan_fft_with_factors<T>(len: usize, factors: &[(usize, usize)], inverse: bool) -> Box<FFTAlgorithm<T>> 
	where T: Signed + FromPrimitive + Copy + Debug + 'static
{
    if factors.len() == 1 {
    	...
    	if factors[0].0 > MIN_RADERS_SIZE {
    		...
    		if factors[0].1 == 1 {
    			...
    			//we have a large prime raised to a power. this is the only case we can't handle well
		...
		if largest_factor > MIN_RADERS_SIZE {
			...
			let factor_fft = plan_fft_with_factors(factor_size, &factors[factors.len()-1..], inverse);
			let other_size = len / factor_size;
			let other_fft = plan_fft_with_factors(other_size, &factors[..factors.len()-1], inverse);

			Box::new(GoodThomasAlgorithm::new(factor_size, factor_fft, other_size, other_fft)) as Box<FFTAlgorithm<T>>
		} else {
