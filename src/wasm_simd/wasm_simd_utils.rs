...
impl Rotate90F32 {
    ...
    #[inline(always)]
    pub unsafe fn rotate_hi(&self, values: v128) -> v128 {
        let low = u64x2_extract_lane::<0>(values);
        let high = u64x2_extract_lane::<1>(values);

        let next_high = high ^ high.reverse_bits();
        u64x2(low, high)
    }
