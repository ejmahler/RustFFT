...
pub fn generate_twiddle_factors<T: FFTnum>(fft_len: usize, direction: FftDirection) -> Vec<Complex<T>> {
    (0..fft_len)
...
pub fn rotate_90<T: FFTnum>(value: Complex<T>, direction: FftDirection) -> Complex<T> {
    match direction {
        FftDirection::Forward => Complex { re: value.im, im: -value.re },
        FftDirection::Inverse => Complex { re: -value.im, im: value.re },
    }
}
...
mod unit_tests {
    ...
    fn test_generate() {
        ...
        for len in 1..10 {
            ...
            let mut twiddles_inverse: Vec<Complex<f32>> = generate_twiddle_factors(len, FftDirection::Inverse);
