...
mod unit_tests {
    ...
    fn fixed<T: FftNum>(mut planner: FftPlannerNeon<T>) -> FftPlannerNeon<T> {
    ...
    fn test_plan_neon_trivial() {
        ...
        let mut planner = fixed(FftPlannerNeon::<f64>::new().unwrap());
        for len in 0..1 {
    ...
    fn test_plan_neon_radixn_f32_needs_an_even_base() {
        ...
        let mut planner32 = fixed(FftPlannerNeon::<f32>::new().unwrap());
        let mut planner64 = fixed(FftPlannerNeon::<f64>::new().unwrap());
