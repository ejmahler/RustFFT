...
pub fn estimate_radix4_cost(len: usize, repeats: usize) -> f32 {
    if len <= 128 {
        (RADIX4_SHORT_CONST + RADIX4_SHORT_SLOPE * (len as f32).powf(RADIX4_SHORT_EXP))
            * repeats as f32
    } else {
        (RADIX4_LONG_CONST + RADIX4_LONG_SLOPE * (len as f32).powf(RADIX4_LONG_EXP))
            * repeats as f32
    }
}
...
pub fn estimate_mixedradix_cost(
    ...
    let twiddle_cost = MIXEDRADIX_CONST + MIXEDRADIX_SLOPE * (len as f32).powf(MIXEDRADIX_EXP);
...
pub fn estimate_mixedradixsmall_cost(
    ...
    let inners_cost = left_fft.cost(right_fft.len()) + right_fft.cost(left_fft.len());
    let twiddle_cost =
        MIXEDRADIXSMALL_CONST + MIXEDRADIXSMALL_SLOPE * (len as f32).powf(MIXEDRADIXSMALL_EXP);
    (inners_cost + twiddle_cost) * repeats as f32
}
...
pub fn estimate_goodthomas_cost(
    ...
    let inners_cost = left_fft.cost(right_fft.len()) + right_fft.cost(left_fft.len());
    let twiddle_cost = GOODTHOMAS_CONST + GOODTHOMAS_SLOPE * (len as f32).powf(GOODTHOMAS_EXP);
    (inners_cost + twiddle_cost) * repeats as f32
}
...
pub fn estimate_goodthomassmall_cost(
    ...
    let inners_cost = left_fft.cost(right_fft.len()) + right_fft.cost(left_fft.len());
    let twiddle_cost =
        GOODTHOMASSMALL_CONST + GOODTHOMASSMALL_SLOPE * (len as f32).powf(GOODTHOMASSMALL_EXP);
    (inners_cost + twiddle_cost) * repeats as f32
}
