...
const MAX_RADIX4_BITS: u32 = 18; // largest size to consider radix 4 an option is 2^16 = 65536
...
impl<T: FftNum> FftPlannerSse<T> {
    ...
    fn build_new_fft(&mut self, recipe: &Recipe, direction: FftDirection) -> Arc<dyn Fft<T>> {
        ...
        match recipe {
            ...
            Recipe::Butterfly11 => {
                if id_t == id_f32 {
                    Arc::new(Butterfly11::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
            ...
            Recipe::Butterfly13 => {
                if id_t == id_f32 {
                    Arc::new(Butterfly13::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
