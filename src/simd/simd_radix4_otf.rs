...
pub struct SimdRadix4OnTheFly<V: SimdVector, T> {
    twiddles: Box<[V]>,
...
impl<V: SimdVector, T: FftNum> SimdRadix4OnTheFly<V, T> {
    ...
    fn transpose(&self, input: &[Complex<T>], output: &mut [Complex<T>]) {
        if self.len > self.base_len {
            bitreversed_transpose::<_, RADIX>(self.base_len, input, output);
        } else {
...
impl<V: SimdVector, T: FftNum> Fft<T> for SimdRadix4OnTheFly<V, T> {
...
impl<V: SimdVector, T> Length for SimdRadix4OnTheFly<V, T> {
    #[inline(always)]
    fn len(&self) -> usize {
        self.len
    }
}
impl<V: SimdVector, T> Direction for SimdRadix4OnTheFly<V, T> {
    #[inline(always)]
...
pub mod test_bodies {
        ...
        V::ScalarType: Float + SampleUniform,
    {
        ...
        let direction = base_fft.fft_direction();
        let fft: SimdRadix4OnTheFly<V, V::ScalarType> = SimdRadix4OnTheFly::new(k, base_fft);
