...
impl<T: FftNum> FftPlannerFcma<T> {
    ...
    ///
    /// Returns `Ok(planner_instance)` if we're compiling for AArch64 and FCMA support was enabled in feature flags.
    /// Returns `Err(())` if FCMA support is not available.
    pub fn new() -> Result<Self, ()> {
        if std::arch::is_aarch64_feature_detected!("fcma") {
            ...
            //
            // We use TypeID to determine if T is f32, f64, or neither. If neither, we don't want to do any FCMA acceleration
            // If it's f32 or f64, then construct and return an FCMA planner instance.
            //
            // All FCMA accelerated algorithms come in separate versions for f32 and f64. The type is checked when a new one is created, and if it does not
            ...
            if id_t == id_f32 || id_t == id_f64 {
                ...
                let prime_butterflies = fcma_prime_butterflies::prime_butterfly_lens();
    ...
    fn build_new_fft(&mut self, recipe: &Recipe, direction: FftDirection) -> Arc<dyn Fft<T>> {
        ...
        match recipe {
            ...
            Recipe::Butterfly1 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly1::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly1::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly2 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly2::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly2::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly3 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly3::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly3::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly4 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly4::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly4::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly5 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly5::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly5::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly6 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly6::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly6::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly8 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly8::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly8::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly9 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly9::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly9::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly10 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly10::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly10::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly12 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly12::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly12::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly15 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly15::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly15::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly16 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly16::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly16::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly24 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly24::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly24::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::Butterfly32 => {
                if id_t == id_f32 {
                    Arc::new(FcmaF32Butterfly32::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(FcmaF64Butterfly32::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            Recipe::PrimeButterfly { len } => unsafe {
                fcma_prime_butterflies::construct_prime_butterfly(*len, direction)
            },
            Recipe::MixedRadix {
                left_fft,
                right_fft,
    ...
    fn design_butterfly_algorithm(&mut self, len: usize) -> Option<Arc<Recipe>> {
        match len {
            ...
            _ => {
                if fcma_prime_butterflies::prime_butterfly_lens().contains(&len) {
...
mod unit_tests {
    ...
    #[test]
    fn test_plan_fcma_trivial() {
        // Length 0 and 1 should use Dft
        let mut planner = FftPlannerFcma::<f64>::new().unwrap();
        for len in 0..1 {
    ...
    #[test]
    fn test_plan_fcma_largepoweroftwo() {
    ...
    fn test_plan_fcma_butterflies() {
        ...
        for len in fcma_prime_butterflies::prime_butterfly_lens() {
            assert_eq!(
    ...
    #[test]
    fn test_plan_fcma_mixedradixsmall() {
    ...
    #[test]
    fn test_plan_fcma_goodthomasbutterfly() {
        let mut planner = FftPlannerFcma::<f64>::new().unwrap();
    ...
    #[test]
    fn test_plan_fcma_bluestein_vs_rader() {
    ...
    fn test_multidimensional_planned_nd<const D: usize>(
        planner: &mut FftPlannerFcma<f32>,
        shape: [usize; D],
    ) {
        // Forward FFT
    ...
    #[test]
    fn test_multidimensional_planned_fcma_2d() {
        let mut planner = FftPlannerFcma::<f32>::new().unwrap();
    ...
    #[test]
    fn test_multidimensional_planned_fcma_3d() {
        let mut planner = FftPlannerFcma::<f32>::new().unwrap();
    ...
    #[test]
    fn test_multidimensional_planned_fcma_4d() {
        let mut planner = FftPlannerFcma::<f32>::new().unwrap();
    ...
    #[test]
    fn test_multidimensional_planned_fcma_5d() {
        let mut planner = FftPlannerFcma::<f32>::new().unwrap();
