...
macro_rules! boilerplate_fft_fcma_oop {
    ($struct_name:ident, $len_fn:expr) => {
        impl<N: FcmaNum, T: FftNum> Fft<T> for $struct_name<N, T> {
        ...
        impl<N: FcmaNum, T> Length for $struct_name<N, T> {
            #[inline(always)]
            fn len(&self) -> usize {
                $len_fn(self)
            }
        }
        impl<N: FcmaNum, T> Direction for $struct_name<N, T> {
            #[inline(always)]
...
pub unsafe fn fcma_fft_helper_immut<T>(
    input: &[T],
    output: &mut [T],
    scratch: &mut [T],
    chunk_size: usize,
...
// so that things like loading twiddle factor registers etc can be lifted out of the loop
#[target_feature(enable = "neon,fcma")]
pub unsafe fn fcma_fft_helper_outofplace<T>(
    input: &mut [T],
    output: &mut [T],
    scratch: &mut [T],
    chunk_size: usize,
...
// so that things like loading twiddle factor registers etc can be lifted out of the loop
#[target_feature(enable = "neon,fcma")]
pub unsafe fn fcma_fft_helper_inplace<T>(
    buffer: &mut [T],
    scratch: &mut [T],
    chunk_size: usize,
    required_scratch: usize,
