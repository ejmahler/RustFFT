...
use super::fcma_vector::{FcmaArray, FcmaArrayMut, FcmaVector, Rotation90};

/// FFT algorithm optimized for power-of-two sizes, FCMA accelerated version.
/// This is designed to be used via a Planner, and not created directly.

pub struct FcmaRadix4<N: FcmaNum, T> {
...
impl<N: FcmaNum, T: FftNum> FcmaRadix4<N, T> {
    /// Constructs a new FcmaRadix4 which computes FFTs of size 4^k * base_fft.len()
    #[inline]
    pub fn new(k: u32, base_fft: Arc<dyn Fft<T>>) -> Self {
        ...
        while cross_fft_len <= len {
            ...
            for i in 0..num_vector_columns {
                for k in 1..ROW_COUNT {
                    unsafe {
                        twiddle_factors.push(FcmaVector::make_mixedradix_twiddle_chunk(
        ...
        Self {
            ...
            rotation: unsafe { FcmaVector::make_rotate90(direction) },
...
boilerplate_fft_fcma_oop!(FcmaRadix4, |this: &FcmaRadix4<_, _>| this.len);

//#[target_feature(enable = "neon,fcma")]
unsafe fn butterfly_4<N: FcmaNum, T: FftNum>(
    data: &mut [Complex<T>],
        ...
        .take(num_ffts / (N::VectorType::COMPLEX_PER_VECTOR * 2))
    {
        ...
        scratcha[1] = FcmaVector::mul_complex(scratcha[1], tw[0]);
        scratcha[2] = FcmaVector::mul_complex(scratcha[2], tw[1]);
        scratcha[3] = FcmaVector::mul_complex(scratcha[3], tw[2]);
        scratchb[1] = FcmaVector::mul_complex(scratchb[1], tw[3]);
        scratchb[2] = FcmaVector::mul_complex(scratchb[2], tw[4]);
        scratchb[3] = FcmaVector::mul_complex(scratchb[3], tw[5]);

        let scratcha = FcmaVector::column_butterfly4(scratcha, *rotation);
        let scratchb = FcmaVector::column_butterfly4(scratchb, *rotation);
...
mod unit_tests {
    ...
    fn test_fcma_radix4_64() {
        for base in [2, 4, 6, 8, 12, 16] {
            ...
            for k in 0..4 {
                test_fcma_radix4_64_with_base(k, Arc::clone(&base_forward));
                test_fcma_radix4_64_with_base(k, Arc::clone(&base_inverse));
            }
        }
    }

    fn test_fcma_radix4_64_with_base(k: u32, base_fft: Arc<dyn Fft<f64>>) {
        let len = base_fft.len() * 4usize.pow(k);
        let direction = base_fft.fft_direction();
        let fft = FcmaRadix4::<f64, f64>::new(k, base_fft);
        check_fft_algorithm::<f64>(&fft, len, direction);
    }

    #[test]
    fn test_fcma_radix4_32() {
        for base in [4, 8, 12, 16] {
            let base_forward = construct_base(base, FftDirection::Forward);
            let base_inverse = construct_base(base, FftDirection::Inverse);
            for k in 0..4 {
                test_fcma_radix4_32_with_base(k, Arc::clone(&base_forward));
                test_fcma_radix4_32_with_base(k, Arc::clone(&base_inverse));
            }
        }
    }

    fn test_fcma_radix4_32_with_base(k: u32, base_fft: Arc<dyn Fft<f32>>) {
        let len = base_fft.len() * 4usize.pow(k);
        let direction = base_fft.fft_direction();
        let fft = FcmaRadix4::<f32, f32>::new(k, base_fft);
