import sys
from matplotlib import pyplot as plt
import numpy as np
from read_bench_series import read_benches
from scipy.optimize import minimize

fnames = sys.argv[1:]
data = read_benches(fnames)


def rms_rel_diff(array1, array2):
   return np.sqrt(np.mean(np.square((array1 - array2))))

mixedradixes_lengths = [(3, 4), (3, 5), (3, 7), (3,13), (3,31), (7, 31), (23, 31), (29,31), (31, 127), (31, 233), (127, 233), (127, 1031), (1031, 2003)]
mixedradixes_large_lengths = mixedradixes_lengths[8:]
mixedradixes_small_lengths = mixedradixes_lengths[0:8]
mixedradixes_cycles = {}
mixedradixes_len = {}
mixedradixes_cycles_twiddling = {}
mix_algos = ["mixedradix", "mixedradixsmall", "goodthomas", "goodthomassmall"]
mixedradixes_inner_cycles = {}
inner_len = []
inner_cycles = []
for mix_lens in mixedradixes_lengths:
    len_a, len_b = mix_lens
    step_len = len_a * len_b
    inner_len.append(step_len)
    benchname1 = f"bench_mixinners_{len_a}_{len_b}"
    cycles1 = data[benchname1]
    benchname2 = f"bench_mixinners_{len_b}_{len_a}"
    cycles2 = data[benchname2]
    mixedradixes_inner_cycles[step_len] = cycles1 + cycles2
    inner_cycles.append(cycles1 + cycles2)


exp_d = {}
slope_d = {}
const_d = {}
legends = []

for mr in mix_algos:
    mixedradixes_cycles[mr] = []
    mixedradixes_len[mr] = []
    mixedradixes_cycles_twiddling[mr] = []
    if mr.endswith("small"):
        lens = mixedradixes_small_lengths
    else:
        lens = mixedradixes_large_lengths
    for fftlens in lens:
        len_a, len_b = fftlens
        benchname = f"bench_{mr}_{len_a}_{len_b}"
        cycles = data[benchname]
        if cycles > mixedradixes_inner_cycles[len_a*len_b]:
            mixedradixes_cycles[mr].append(cycles)
            mixedradixes_len[mr].append(len_a*len_b)
            mixedradixes_cycles_twiddling[mr].append(cycles - mixedradixes_inner_cycles[len_a*len_b])

    plt.figure(10)
    plt.title("Total time")
    plt.loglog(mixedradixes_len[mr], mixedradixes_cycles[mr])
    plt.figure(11)
    mr_overhead = np.array(mixedradixes_cycles_twiddling[mr])
    plt.title("Overhead")
    plt.loglog(mixedradixes_len[mr], mr_overhead,'*')

    print(mr)
    print(mr_overhead)
    print(mixedradixes_len[mr])
    k, m = np.polyfit(np.log(mixedradixes_len[mr]), np.log(mr_overhead), 1)
    fit = np.exp(m)*np.array(mixedradixes_len[mr])**k
    plt.loglog(mixedradixes_len[mr], fit)
    legends.append(mr)
    legends.append(f"fit to {mr}")

    slope_d[mr] = np.exp(m)
    exp_d[mr] = k

print("// --- Begin code generated by tools/fit_mixedradixes.py --- \n")

for algo in mix_algos:
    print(f"const {algo.upper()}_SLOPE: f32 = {slope_d[algo]:.5f};")
    print(f"const {algo.upper()}_EXP: f32 = {exp_d[algo]:.5f};")
    print("")
    print(f"pub fn estimate_{algo}_cost(len: usize, left_fft: &Arc<Recipe>, right_fft: &Arc<Recipe>, repeats: usize) -> f32 {{")
    print(f"    let inners_cost = left_fft.cost(right_fft.len()) + right_fft.cost(left_fft.len());")
    print(f"    let twiddle_cost = {algo.upper()}_SLOPE * (len as f32).powf({algo.upper()}_EXP);")
    print(f"    (inners_cost + twiddle_cost) * repeats as f32")
    print("}")
    print("")

print("// --- End code generated by tools/fit_mixedradixes.py --- \n")

plt.figure(10)
plt.loglog(inner_len, inner_cycles)

plt.figure(11)
plt.legend(legends)
plt.show()

